...
```{r}
C_info <- data.frame(c(1, 1.5, 1.5, 1, 1, 1.5, 5.5, 6, 6), c(10, 10, 9.5, 5, 4.5, 4.5, 5.5, 5.5, 6), rep(0.5, 9), rep(NA, length(pts)))
colnames(C_info) <- c("X", "Y", "lambda", "lambda_hat")

est_lambda <- function(x, y, bw, lambda, window, lattice){
  ...
  lambda_hat(x, y, final, bw, myRegion, myArea)
}
mu <- 0
for(i in 1:100){
  mu <- (mu * (i - 1) + est_lambda(1, 10, 0.555, lambda_c, my_window, my_lattice )) / i 
}
```
#Simulation D
...
Z <- matrix(mapply(lambda_hat, XforZ, Y, MoreArgs = list(my_set = final, bw = 4, my_region = myRegion, my_Area = myArea)), nrow = 14, byrow = T)
contour(X, Y, Z)
```
...
d_EBB <-
  d %>%
  filter(bog == "EBB" & plantX > 500 & plantX < 10500 & plantY > 500 & plantX < 10500)
sx <- rep(1:10, 10)
sy <- rep(1:10, each = 10)
plot(sx, sy, xlab = "", ylab = "", pch = 16)
...
d_NO5 <-
  d %>%
  filter(bog == "NO5" & plantX > 500 & plantX < 10000 & plantY > 500 & plantY < 10000)
sx <- rep(1:10, 10)
sy <- rep(1:10, each = 10)
plot(sx, sy, xlab = "", ylab = "", pch = 16)
...
contour(X, Y, Z, levels = seq(0, 0.4, by = 0.05), xlim = c(1, 10), ylim = c(1, 10), zlim = c(0, 5))
points(my_lattice, col = "black", pch = 16)
